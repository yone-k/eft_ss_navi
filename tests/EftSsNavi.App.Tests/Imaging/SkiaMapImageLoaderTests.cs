...
namespace EftSsNavi.App.Tests.Imaging;
...
public sealed class SkiaMapImageLoaderTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"eft-ss-navi-{Guid.NewGuid():N}", "missing.png");
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"eft-ss-navi-{Guid.NewGuid():N}");
