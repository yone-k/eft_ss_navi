...
public sealed class ApplicationPublishTests
{
    ...
    public async Task ShouldCopyApplicationXamlResourcesToPublishDirectory()
    {
        ...
        var publishDirectory = Path.Combine(
            Path.GetTempPath(),
            $"eft-ss-navi-publish-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Assert.True(
                File.Exists(Path.Combine(publishDirectory, "EftSsNavi.App.pri")),
                diagnosticOutput);
            Assert.True(
    ...
    public async Task ShouldPublishBundledMapAssetsAndThirdPartyNotices()
    {
        ...
        var publishDirectory = Path.Combine(
            Path.GetTempPath(),
            $"eft-ss-navi-assets-{Guid.NewGuid():N}");
        var buildDirectory = Path.Combine(
            Path.GetTempPath(),
            $"eft-ss-navi-build-{Guid.NewGuid():N}");
